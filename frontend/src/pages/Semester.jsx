  ...
  return (
    <>
      <>
        ...
        <div class="body-font bg-blue-700 min-h-[90vh] w-screen w-full rounded-lg " style={{
          ...
          <div class="container px-5 py-24 mx-auto">
            ...
            <div class="flex flex-wrap w-full mb-20 flex-col items-center text-center">
              <h1 class="sm:text-3xl text-2xl font-medium title-font mb-2 text-white">Select Your Course</h1>
              <p class="lg:w-1/2 w-full leading-relaxed font-medium text-gray-100 text-2xl">Semester {semesterId % 8 === 0 ? 8 : semesterId % 8}
                |  {branch?.name || "Branch"}</p>
            </div>
            <div class="flex flex-wrap -m-4">

              {subs.map((sub) => (<>
                <div key={sub.id} class="w-full p-4 md:w-1/2 xl:w-1/3">
                  <Link to={`/branch/${branchId}/semester/${semesterId}/subject/${sub.id}`}>
                    <div class="rounded-lg  bg-white border border-gray-200 p-6">


                      <h2 class="title-font mb-2 text-lg font-medium text-gray-900">{sub.name}</h2>
